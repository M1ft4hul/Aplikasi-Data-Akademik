...
class Report extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (!empty($cek) && $stts == 'admin') {
            ...
            $data['desc'] = "Data Laporan per Kategori, Menampilkan data buku per Kategori";
    ...
    public function pengembalian()
    {
        ...
        if (!empty($cek) && $stts == 'admin') {
            /*layout*/
            $data['title'] = 'Laporan';
            $data['pointer'] = "Report/pengembalian";
            $data['classicon'] = "fa fa-book";
            $data['main_bread'] = "Data Report";
            $data['sub_bread'] = "Daftar Report";
            $data['desc'] = "Data Laporan per Bulan, Menampilkan data buku per Bulan";
            /*data yang ditampilkan*/
    ...
    public function excel1()
    {
        ...
        if (count($data_buku) > 0) {
            ...
            $val = array("NO", "ID Anggota ", "Nama ", "Kelas", "Kode Kartu", "Judul Buku", "No. Buku", "Tanggal Pinjam", "Tanggal Kembali", "Tanggal Dikembalikan", "Telat", "Denda");
            ...
            for ($a = 0; $a < 12; $a++) {
                ...
                $objPHPExcel->getActiveSheet()->getColumnDimension('L')->setWidth(15); //kontak
                $style = array(
                    'alignment' => array(
                        'horizontal' => PHPExcel_Style_Alignment::VERTICAL_CENTER,
                    )
                ...
                if ($frow->status == 1) {
                    ...
                    foreach ($data_kelas->result() as $key => $kelas) {
                        if ($kelas->id_kelas == $idkl) {
                            $objset->setCellValue("D" . $baris, $kelas->kelas); //membaca data kelas                      
                        }
                    }
                    foreach ($data_anggota->result() as $key => $anggota) {
                        $ida = $frow->id_anggota;
                        if ($anggota->id_anggota == $ida) {
                            $objset->setCellValue("E" . $baris, $anggota->kode_kartu); //membaca data nama
                        }
                    }
                                ...
                                if ($buku->id_buku == $idbku) {
                                    foreach ($data_detail_buku->result() as $key => $dtbuku) {
                                        if ($dtbuku->no_buku == $nobku && $dtbuku->id_buku == $idbku) {
                                            $objset->setCellValue("F" . $baris, $buku->judul); //membaca data kontak
                                            $objset->setCellValue("G" . $baris, $dtbuku->no_buku); //membaca data kontak  
                                        }
                                    }
                                }
                            }
                    ...
                    $objset->setCellValue("H" . $baris, $frow->tgl_pinjam); //membaca data kontak   
                    $objset->setCellValue("I" . $baris, $frow->tgl_kembali);
                    foreach ($data_kembali->result() as $key => $kembali) {
                        if ($kembali->id_pinjam == $pinjam) {
                            $objset->setCellValue("J" . $baris, $kembali->tgl_dikembalikan); //membaca data alamat
                            $objset->setCellValue("K" . $baris, $kembali->terlambat);
                            $objset->setCellValue("L" . $baris, $kembali->denda);
                        }
                    }
                    //Set number value
                    $objPHPExcel->getActiveSheet()->getStyle('L1:L' . $baris)->getNumberFormat()->setFormatCode('0');
