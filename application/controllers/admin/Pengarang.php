...
class Pengarang extends MY_Controller
{
	...
	public function index()
	{
		...
		if (!empty($cek) && $stts == 'admin') {
			...
			$tmp['content'] = $this->load->view('admin/pengarang/View_pengarang', $data, TRUE);
	...
	public function create()
	{
		...
		if (!empty($cek) && $stts == 'admin') {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				//$data['isi']=$this->Anggota_model->get_all();
				//$data['js']=$this->load->view('admin/anggota/js');
				$tmp['content'] = $this->load->view('admin/pengarang/Create_pengarang', $data, TRUE);
				$this->load->view('admin/layout', $tmp);
			} else {
				$data = array(
					'id_pengarang' => '',
	...
	public function edit()
	{
		...
		if (!empty($cek) && $stts == 'admin') {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				//$data['data_kelas'] = $this->Buku_model->getAllData("tb_kelas");
				//$data['data_agama'] = $this->Buku_model->getAllData("tb_agama");
				$tmp['content'] = $this->load->view('admin/pengarang/Edit_pengarang', $data, true);
				$this->load->view('admin/layout', $tmp);
			}
		} else {
			header('location:' . base_url() . 'login/auth');
