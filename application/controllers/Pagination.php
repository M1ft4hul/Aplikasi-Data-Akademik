...
class Pagination extends CI_Controller
{
    public function index()
    {
        ...
        $this->load->view('pagination/header', $data);
    ...
    {
        // manggil model
        $this->load->model('Pagination_models', 'halaman');
        ...
        // config pagination
        $config['base_url'] = 'http://localhost/perpus/pagination/data';
        $this->db->like('nama', $data['pencarian']);
        $this->db->from('register');
        $config['total_rows'] = $this->db->count_all_results();
        $data['total_rows'] = $config['total_rows'];
        $config['per_page'] = 5; // ini adalah jumlah data yang akan mau ditampilkan di halaman nya 
        ...
        $data['pagination'] = $this->halaman->getBuku($config['per_page'], $data['start'], $data['pencarian']);
